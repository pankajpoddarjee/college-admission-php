...
//$fetch_query = mysqli_query($connection, "select * from exams where is_active = 1 limit $page_start, $limit_page");
...
$records = [];
$activeStatus = 1;
//$strsql="select * from exams where is_active = :is_active limit $page_start, $limit_page";
// $strsql="SELECT c.id, c.exam_name, c.short_name, c.college_type_id, c.country_id, co.country_name, c.state_id, s.state_name, c.city_id, ci.city_name, c.banner_img, c.logo_img, c.folder_name, c.file_name,c.slug FROM exams c 
// JOIN countries co ON c.country_id=co.id 
// JOIN states s ON c.state_id=s.id 
// JOIN cities ci ON c.city_id=ci.id 
// WHERE  c.is_active=:is_active order by c.exam_name OFFSET $offset ROWS
...
// $sql = "SELECT exams.*, countries.country_name, states.state_name, cities.city_name, college_course_details.stream_id, college_course_details.course_type_id FROM exams 
// Left JOIN college_course_details on college_course_details.college_id = exams.id 
...
";
        ...
        //     $sql .= " AND exams.undertaking_id = :undertaking_id";
        // }
        // if (!empty($masterSlugRecord['university_id'])) {
        //     $sql .= " AND exams.university_id = :university_id";
        // }
        // if (!empty($masterSlugRecord['college_type_id'])) {
        //     $sql .= " AND exams.college_type_id = :college_type_id";
        // }
        ...
        //     $sql .= " AND exams.tags IN :tags";
        // }
        $sql .= " order by exams.exam_name OFFSET $offset ROWS FETCH NEXT $limit ROWS ONLY";
...
if(count($records) > 0){
    ...
    foreach ($records as  $rec) {
        ...
        $exam_name = !empty($rec["exam_name"])?$rec["exam_name"]:"";
        $short_name = !empty($rec["short_name"])?'['.$rec["short_name"].']':"";
        $banner_img = !empty($rec["banner_img"])?BASE_URL_UPLOADS."/exam/banner_image/".$rec["banner_img"]:BASE_URL."/images/no-image.jpg";
        $logo_img = !empty($rec["logo_img"])?BASE_URL_UPLOADS."/exam/logo_image/".$rec["logo_img"]:BASE_URL."/images/no-logo.jpg";
        $city_name = !empty($rec["city_name"])?$rec["city_name"]:"";
        $state_name = !empty($rec["state_name"])?$rec["state_name"]:"";
        $output .='<div class="col-md-3 mb-4"><div class="img-thumbnail"><div class="row"><div class="col-12"><a href="'.$college_url.'"><div class="listing_banner"><img class="img-fluid border p-1" src="'.$banner_img.'"></div><div class="ms-2 listing_logo"><img class="bg-white p-1 rounded d-block" src="'.$logo_img.'"></div><div class=""><span class="text-danger">'.$exam_name.' '.$short_name.'</span></div></a></div></div></div></div></div>';
    }
}
