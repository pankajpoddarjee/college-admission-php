...
if(count($collegeRecord)>0){
    ...
    $fetchallqry = "SELECT * FROM notice_colleges WHERE college_id=$college_id AND notice_year = $current_year AND notice_category='Merit List' AND is_active=1 order by notice_date desc, id desc" ;
    $stmt = $dbConn->prepare($fetchallqry);
    $stmt->execute();
        ...
        <div class="container-xxl">
            <div class="row">
                <div class="col-md-9">
                	<div class="row">
                    	<div class="col-md-12">
                            <h4 class="alert alert-info" title="<?php echo strtolower($college_name);?> admission <?php echo CURRENT_YEAR_DISPLAY_C_PAGE;?>" style="font-family:Oswald"><i class="fa-solid fa-bullhorn"></i> Merit / Admission List <?php echo CURRENT_YEAR_DISPLAY_C_PAGE;?></h4>
                        </div>
                    </div>
                    ...
                    <?php 
                                $sql = "select distinct notice_year from notice_colleges where college_id = $college_id AND is_active=1 AND notice_category='Merit List' order by notice_year desc";
    ...
    <script>
        function showArchiveData(year){
            ...
            $.ajax({
                    type: "get",
                   // async: false,
                    url: "<?php echo BASE_URL;?>/Get_archived_notice.php?year=" + year +'&type=college&college_id='+ '<?php echo $college_id?>'+'&for=merit_list',
