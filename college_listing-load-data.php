...
";
        ...
        // }
        if (!empty($masterSlugRecord['stream_id'])) {
            $streamIds = explode(',', $masterSlugRecord['stream_id']); // Split the comma-separated string into an array
            $placeholders = [];
            foreach ($streamIds as $index => $streamId) {
                $placeholders[] = ":stream_id_$index"; // Create unique named placeholders
            }
            $sql .= " AND UniqueCourseDetails.stream_id IN (" . implode(',', $placeholders) . ")";
        }
        ...
        if (!empty($masterSlugRecord['stream_id'])) {
            foreach ($streamIds as $index => $streamId) {
                $stmt->bindValue(":stream_id_$index", $streamId, PDO::PARAM_INT);
            }
        }
        if (!empty($masterSlugRecord['course_type_id'])) {
