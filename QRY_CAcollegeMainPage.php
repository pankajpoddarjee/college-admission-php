...
<?php 
$qry = "SELECT c.id, c.slug as collegeSlug, c.college_name, c.short_name, ct.college_type_name, c.eshtablish, c.college_type_id, ut.undertaking_name, ct.college_type_name, c.other_name, c.country_id, co.country_name, c.state_id, s.state_name, c.city_id, ci.city_name, c.college_code, c.landmark, university_id,  c.accreditation, c.address, c.website_url, c.website_display, c.email, c.email2, c.phone, c.folder_name, c.file_name, c.logo_img, c.banner_img FROM colleges c 
LEFT JOIN college_types ct ON c.college_type_id=ct.ID 
JOIN countries co ON c.country_id=co.ID 
JOIN states s ON c.state_id=s.ID 
JOIN cities ci ON c.city_id=ci.ID 
-- JOIN universities un ON c.university_id=un.ID 
LEFT JOIN undertakings ut ON c.undertaking_id=ut.ID "; 
if($uri == 'college'){
...
$university_arr = explode(',',$record['university_id']);
$university_name_arr = [];
if(isset($university_arr)){
    foreach ($university_arr as $value) {
        $university_name_arr[] = getUniversityNameById($value);
    }
}
$university_name =implode(', ', $university_name_arr);
//$university_short_name ="ppp";
