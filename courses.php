...
if(count($collegeRecord)>0){
    ...
    $records = [];
    $fetchallqry = "SELECT ccd.*, s.stream_name, s.short_name, ct.course_type_name, ct.course_code FROM college_course_details as ccd 
    LEFT JOIN streams as s On s.id = ccd.stream_id 
    JOIN course_types as ct ON ct.id = ccd.course_type_id 
    WHERE ccd.college_id=$college_id AND ccd.is_active=1 
     order by  ct.course_type_name desc, s.stream_name asc" ;
    $stmt = $dbConn->prepare($fetchallqry);
    $stmt->execute();
                            ...
                            <?php 
                                ...
                                ?>
                                    ...
                                    foreach ($courses as $course) {  
                                        ...
                                        usort($arrSubject, function ($a, $b) {
                                            return strcasecmp($a, $b); // Case-insensitive comparison
                                        });
                                        // $arrSubject = natcasesort($arrSubjectTemp); 
