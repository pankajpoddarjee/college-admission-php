...
$(".status-change").click(function() {
    ...
    $.ajax({
        type: "get",
        async: false,
        url: "Save_notice_college.php?statusid=" + rid + "&status=" + status,
        dataType: "json",
        success: function(data) {
            if (data.status == 1) {
                alert(data.msg);
...
$('body').on('click', '.get-record', function(event) {
    ...
    $.ajax({
        type: "get",
        // async: false,
        url: "Save_notice_college.php?eid=" + eid,
        dataType: "json",
        success: function(data) {
            if (data.status == 1) {
...
$('body').on('click', '.get-attachment-record', function(event) {
    ...
    $('#attch_college_id').val(cid);
    $('#attch_notice_id').val(nid);
    $('#college_name').text(cname);
    ...
    $("#base_directory").val('../../uploads/notices/college/' + cid);
...
function fetchOnlyDirectoryStructure(cid) {
    $.ajax({
        url: "getOnlyDirectory.php?id=" + cid + "&from=college",
        method: 'GET',
        dataType: 'json',
        success: function(data) {
            console.log(data);
...
function fetchSubDirectoryStructure(dir) {
    var cid = $('#attch_college_id').val();
    var dir = dir;
    $.ajax({
        url: "getSubDirectory.php?id=" + cid + "&from=college&dir=" + dir,
        method: 'GET',
        dataType: 'json',
        success: function(data) {
            console.log(data);
...
function fetchFilenameOfDirectory(dir) {
    ...
    var college_id = $('#attch_college_id').val();
    var year_directory = $('#year_directory').val();

    $.ajax({
        url: "getAttachmentFile.php?notice_id=" + notice_id + "&from=college&id=" + college_id + "&year_directory=" + year_directory + "&dir=" + dir,
        method: 'GET',
        dataType: 'json',
        success: function(data) {
            console.log(data);
            ...
            if (data.length > 0) {
                for (let i = 0; i < data.length; i++) {
                    ...
                    var id = data[i].id;
                    html += '<tr>' +
                        '<td class="text-left align-middle"><a href="' + filePath + '" target="_blank">' + fileName + '</a></<td>' +
                        '<td class="text-center align-middle"><a class="btn btn-danger btn-sm" href="javascript:void(0)" data-toggle="tooltip" data-placement="top" title="Delete Attachment" onclick="removeAttachmentFile(\'' + dirPath + '\', \'' + dir + '\', \'' + id + '\')">' +
...
function fetchDirectoryStructure(cid) {
    $.ajax({
        url: "getDirectory.php?id=" + cid + "&from=college",
        method: 'GET',
        dataType: 'json',
        success: function(data) {
...
$('body').on('click', '#save-attachment-notice-button', function(event) {
    ...
    $.ajax({
        ...
        url: "Save_attachments.php?notice_id=" + notice_id + "&from=college&id=" + college_id,
        data: formData,
        dataType: "json",
        success: function(data) {
            // alert(JSON.stringify(data))
            ...
            if (data.status == 1) {
                ...
                fetchDirectoryStructure(college_id);
...
function verifyInputAttachment() {
    ...
    if ($.trim($("#year_directory").val()) == "") {
        toastr.error("Select College Directory");
        ...
        if (year_directory) {
            if (new_directory != '' && typeof new_directory === 'string') {
                $.ajax({
                    type: "get",
                    async: false,
                    url: "Save_attachments.php?newDirectory=" + new_directory + "&from=college&id=" + college_id + "&year_directory=" + year_directory,
                    dataType: "json",
                    success: function(data) {
                        if (data.status == 1) {
        ...
        if (year_directory) {
            if (year_directory != '' && typeof year_directory === 'string') {
                $.ajax({
                    type: "get",
                    async: false,
                    url: "Save_attachments.php?newYearDirectory=" + year_directory + "&from=college&id=" + college_id,
                    dataType: "json",
                    success: function(data) {
                        if (data.status == 1) {
                            // Check if the option already exists to avoid duplication
                            if ($("#year_directory option[value='" + year_directory + "']").length === 0) {
                                ...
                                element.trigger('change'); // Ensure Select2 updates the UI
                            }
...
function removeAttachmentFile(file_path, dir, id) {
    ...
    $.ajax({
        ...
        cache: false,
        //url: "Save_attachments.php",
        url: "Save_attachments.php?removeAttachmentFile=" + file_path + "&from=college&dir=" + dir,
        // data: formData,
        dataType: "json",
        success: function(data) {
            // alert(JSON.stringify(data))
