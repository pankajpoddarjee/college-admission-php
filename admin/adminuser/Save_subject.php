...
if (isset($_GET["eid"])) {
    ...
    $sql = "select id,subject_name,is_active from subjects WHERE id=:id";
...
if (isset($_GET["statusid"])) {
    ...
    $sql = "UPDATE subjects SET is_active = :is_active WHERE id = :id";
    $stmt = $dbConn->prepare($sql);
...
$subject_name = sanitize_string($_POST["subject_name"]);
...
if ($action == "edit" && !empty($record_id) && isset($record_id)) {
    $existQuery = $dbConn->prepare(
        "select * from subjects WHERE subject_name=:subject_name and id!=:id"
    );
    $existQuery->bindParam(":subject_name", $subject_name, PDO::PARAM_STR);
    $existQuery->bindParam(":id", $record_id, PDO::PARAM_INT);
    $existQuery->execute();
    $Record = $existQuery->fetch(PDO::FETCH_ASSOC);
    if ($Record) {
        $statusarr["status"] = 0;
        $statusarr["msg"] = "This subject already exist";
        echo json_encode($statusarr);
        return;
    } else {
        $sql =
            "UPDATE subjects SET subject_name = :subject_name,updated_at = :updated_at,updated_by = :updated_by WHERE id = :id";
        $stmt = $dbConn->prepare($sql);
    ...
    if ($Record) {
        ...
        $active_status = 1;
        $sql =
            "INSERT INTO subjects (subject_name, created_at, created_by, is_active) VALUES (:subject_name, :created_at,:created_by, :is_active)";
        $stmt = $dbConn->prepare($sql);
        $stmt->bindParam(":subject_name", $subject_name, PDO::PARAM_STR);
