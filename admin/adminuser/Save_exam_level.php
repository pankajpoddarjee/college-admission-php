...
if (isset($_GET["eid"])) {
    ...
    $sql = "select id,exam_level_name,short_name,is_active from exam_levels WHERE id=:id";
...
if (isset($_GET["statusid"])) {
    ...
    $sql = "UPDATE exam_levels SET is_active = :is_active WHERE id = :id";
    $stmt = $dbConn->prepare($sql);
...
$exam_level_name = sanitize_string($_POST["exam_level_name"]);  
...
if ($action == "edit" && !empty($record_id) && isset($record_id)) {
    $existQuery = $dbConn->prepare(
        "select * from exam_levels WHERE exam_level_name=:exam_level_name and id!=:id"
    );
    $existQuery->bindParam(":exam_level_name", $exam_level_name, PDO::PARAM_STR);
    ...
    if ($Record) {
        ...
        $sql =
            "UPDATE exam_levels SET exam_level_name = :exam_level_name,short_name = :short_name,updated_at = :updated_at,updated_by = :updated_by WHERE id = :id";
        $stmt = $dbConn->prepare($sql);
        $stmt->bindParam(":exam_level_name", $exam_level_name, PDO::PARAM_STR);
    ...
    $existQuery = $dbConn->prepare(
        "select * from exam_levels WHERE exam_level_name=:exam_level_name "
    );
    $existQuery->bindParam(":exam_level_name", $exam_level_name, PDO::PARAM_STR);
    //$existQuery->bindParam(":short_name", $short_name, PDO::PARAM_STR);
    $existQuery->execute();
    ...
    if ($Record) {
        ...
        $active_status = 1;
        $sql =
            "INSERT INTO exam_levels (exam_level_name,short_name, created_at, created_by, is_active) VALUES (:exam_level_name,:short_name, :created_at,:created_by, :is_active)";
        $stmt = $dbConn->prepare($sql);
        $stmt->bindParam(":exam_level_name", $exam_level_name, PDO::PARAM_STR);
        $stmt->bindParam(":short_name", $short_name, PDO::PARAM_STR);
