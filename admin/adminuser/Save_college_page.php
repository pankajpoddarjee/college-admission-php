...
if (isset($_GET["eid"])) {
    ...
    $sql = "select * from page_colleges WHERE id=:id";
...
if (isset($_GET["statusid"])) {
    ...
    $sql = "UPDATE page_colleges SET is_active = :is_active WHERE id = :id";
    $stmt = $dbConn->prepare($sql);
...
if ($action == "edit" && !empty($record_id) && isset($record_id)) {
    $existQuery = $dbConn->prepare(
        "select * from page_colleges WHERE slug=:slug and id!=:id"
    );
    ...
    if ($existRecord) {
        ...
        if(isset($_FILES['html_page']['name']) && !empty($_FILES['html_page']['name'])){
            ...
            if (empty($errors)) {
                ...
                $parentDirectory = '../../uploads/pages/college'; // Specify the path to the parent folder
                //$newDirectoryName = str_replace(' ', '_', strtolower(getcollegeNameById($college_id)." ".$college_id));
                $newDirectoryName = $college_id;
                ...
                $notice_file_name =  "pages/college/".$newDirectoryName.'/'.$record_id.'-page.'.$ext;
        ...
        $sql = 
            "UPDATE page_colleges SET page_title = :page_title, college_id = :college_id , html_page = :html_page, description = :description, tags = :tags, updated_at = :updated_at,updated_by = :updated_by WHERE id = :id";
        $stmt = $dbConn->prepare($sql);
    ...
    $existQuery = $dbConn->prepare(
        "select * from page_colleges WHERE slug=:slug "
    );
    $existQuery->bindParam(":slug", $slug, PDO::PARAM_STR);
    $existQuery->execute();
    $existRecord = $existQuery->fetch(PDO::FETCH_ASSOC);
    if ($existRecord) {
        ...
        $is_active = 1;
        $sql =
            "INSERT INTO page_colleges (page_title,college_id,description, tags, created_at, created_by, is_active) VALUES (:page_title,:college_id,:description, :tags,:created_at,:created_by, :is_active)";
        $stmt = $dbConn->prepare($sql);
        $stmt->bindParam(":page_title", $page_title, PDO::PARAM_STR);
        $stmt->bindParam(":college_id", $college_id, PDO::PARAM_INT);
        $stmt->bindParam(":description", $description, PDO::PARAM_STR);
        ...
        if ($stmt->execute()) {
            ...
            if(isset($_FILES['html_page']['name']) && !empty($_FILES['html_page']['name'])){
                ...
                if (empty($errors)) {
                    ...
                    //$notice_file_name = "notice-".$id.'.'.$ext;
                        ...
                        //$upload_path = '../../uploads/notices/college/'.$college_name.'/';
                        $notice_file_name =  "pages/college/".$newDirectoryName.'/'.$id.'-page.'.$ext;
            ...
            $slug = "pages/college/".$slug;
            $slug= $slug."-".$id;
          

           
            
            $sql = "UPDATE page_colleges SET slug = :slug, html_page = :html_page WHERE id = :id";           
            $stmt = $dbConn->prepare($sql);
