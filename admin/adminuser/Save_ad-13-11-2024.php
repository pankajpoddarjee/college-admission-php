...
if ($action == "edit" && !empty($record_id) && isset($record_id)) {
        ...
        $sql = 
            "UPDATE ads SET client_id = :client_id,ad_for = :ad_for,ad_link = :ad_link, alt = :alt , ad_image = :ad_image,from_date = :from_date,to_date = :to_date,location = :location, updated_at = :updated_at,updated_by = :updated_by WHERE id = :id";
        $stmt = $dbConn->prepare($sql);
    ...
    $dbConn= NULL;
        ...
        $is_active = 1;
        $sql =
            "INSERT INTO ads (client_id,ad_for, ad_link,alt,from_date,to_date,location, created_at, created_by, is_active) VALUES (:client_id,:ad_for, :ad_link,:alt,:from_date,:to_date,:location,:created_at,:created_by, :is_active)";
        $stmt = $dbConn->prepare($sql);
        $stmt->bindParam(":client_id", $client_id, PDO::PARAM_INT);
        $stmt->bindParam(":ad_for", $ad_for, PDO::PARAM_STR);
        $stmt->bindParam(":ad_link", $ad_link, PDO::PARAM_STR);
