...
if (isset($_GET["eid"])) {
    ...
    $sql = "select * from notice_universities WHERE id=:id";
...
if (isset($_GET["statusid"])) {
    ...
    $sql = "UPDATE notice_universities SET is_active = :is_active WHERE id = :id";
    $stmt = $dbConn->prepare($sql);
...
if ($action == "edit" && !empty($record_id) && isset($record_id)) {
    $existQuery = $dbConn->prepare(
        "select * from notice_universities WHERE slug=:slug and id!=:id"
    );
    ...
    if ($existRecord) {
        ...
        if(isset($_FILES['page_link']['name']) && !empty($_FILES['page_link']['name'])){
            ...
            if (empty($errors)) {
                ...
                $newDirectoryName = $university_id.'/'.$notice_year;
        ...
        $sql = 
            "UPDATE notice_universities SET course_for = :course_for, notice_category = :notice_category, notice_title = :notice_title, university_id = :university_id, notice_type = :notice_type,page_link = :page_link, url_link = :url_link, has_attachment = :has_attachment, notice_date = :notice_date,is_new = :is_new,is_active = :is_active, description = :description, tags = :tags,  notice_year = :notice_year, updated_at = :updated_at,updated_by = :updated_by WHERE id = :id";
        $stmt = $dbConn->prepare($sql);
    ...
    $existQuery = $dbConn->prepare(
        "select * from notice_universities WHERE slug=:slug "
    );
    $existQuery->bindParam(":slug", $slug, PDO::PARAM_STR);
    $existQuery->execute();
    $existRecord = $existQuery->fetch(PDO::FETCH_ASSOC);
    if ($existRecord) {
        ...
        //$active_status = 1;
        $sql =
            "INSERT INTO notice_universities (course_for,notice_category,notice_title,university_id,notice_type,url_link,has_attachment, notice_date,is_new, description, tags,notice_year,created_at, created_by, is_active) VALUES (:course_for,:notice_category, :notice_title, :university_id,:notice_type, :url_link,:has_attachment, :notice_date,:is_new, :description, :tags,:notice_year,:created_at,:created_by, :is_active)";
        $stmt = $dbConn->prepare($sql);
        $stmt->bindParam(":course_for", $course_for, PDO::PARAM_STR);
        $stmt->bindParam(":notice_category", $notice_category, PDO::PARAM_STR);
        $stmt->bindParam(":notice_title", $notice_title, PDO::PARAM_STR);       
        ...
        if ($stmt->execute()) {
            ...
            $slug = "notice/university/".$slug;
            $slug= $slug."-".$id;
            
            $sql = "UPDATE notice_universities SET slug = :slug, page_link = :page_link, url_link = :url_link WHERE id = :id";           
            $stmt = $dbConn->prepare($sql);
            $stmt->bindParam(":slug", $slug, PDO::PARAM_STR);
            $stmt->bindParam(":page_link", $page_link, PDO::PARAM_STR);
            $stmt->bindParam(":url_link", $url_link, PDO::PARAM_STR);
