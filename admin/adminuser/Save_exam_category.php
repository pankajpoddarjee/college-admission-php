...
if (isset($_GET["eid"])) {
    ...
    $sql = "select id,exam_category_name,short_name,is_active from exam_categories WHERE id=:id";
...
if (isset($_GET["statusid"])) {
    ...
    $sql = "UPDATE exam_categories SET is_active = :is_active WHERE id = :id";
    $stmt = $dbConn->prepare($sql);
...
if ($action == "edit" && !empty($record_id) && isset($record_id)) {
    $existQuery = $dbConn->prepare(
        "select * from exam_categories WHERE exam_category_name=:exam_category_name and id!=:id"
    );
    $existQuery->bindParam(":exam_category_name", $exam_category_name, PDO::PARAM_STR);
    ...
    if ($Record) {
        $statusarr["status"] = 0;
        $statusarr["msg"] = "This exam category already exist";
        echo json_encode($statusarr);
        return;
    } else {
        $sql =
            "UPDATE exam_categories SET exam_category_name = :exam_category_name,short_name = :short_name,updated_at = :updated_at,updated_by = :updated_by WHERE id = :id";
        $stmt = $dbConn->prepare($sql);
    ...
    $existQuery = $dbConn->prepare(
        "select * from exam_categories WHERE exam_category_name=:exam_category_name"
    );
    $existQuery->bindParam(":exam_category_name", $exam_category_name, PDO::PARAM_STR);
   // $existQuery->bindParam(":short_name", $short_name, PDO::PARAM_STR);
    $existQuery->execute();
    ...
    if ($Record) {
        ...
        $active_status = 1;
        $sql =
            "INSERT INTO exam_categories (exam_category_name,short_name, created_at, created_by, is_active) VALUES (:exam_category_name,:short_name, :created_at,:created_by, :is_active)";
        $stmt = $dbConn->prepare($sql);
        $stmt->bindParam(":exam_category_name", $exam_category_name, PDO::PARAM_STR);
        $stmt->bindParam(":short_name", $short_name, PDO::PARAM_STR);
