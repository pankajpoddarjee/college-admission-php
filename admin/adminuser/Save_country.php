...
if (isset($_GET["eid"])) {
    ...
    $sql = "select id,country_name,short_name,is_active from countries WHERE id=:id";
...
if ($action == "edit" && !empty($record_id) && isset($record_id)) {
    ...
    if ($Record) {
        ...
        $sql =
            "UPDATE countries SET country_name = :country_name,short_name = :short_name,updated_at = :updated_at,updated_by = :updated_by WHERE id = :id";
        $stmt = $dbConn->prepare($sql);
    ...
    if ($Record) {
        ...
        $active_status = 1;
        $sql =
            "INSERT INTO countries (country_name,short_name, created_at, created_by, is_active) VALUES (:country_name, :short_name, :created_at,:created_by, :is_active)";
        $stmt = $dbConn->prepare($sql);
        $stmt->bindParam(":country_name", $country_name, PDO::PARAM_STR);
        $stmt->bindParam(":short_name", $short_name, PDO::PARAM_STR);
