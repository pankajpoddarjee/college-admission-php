...
$stream_id = isset($_POST['stream_id']) ? implode(',', $_POST['stream_id']) : 0;
$college_type_id = sanitize_string($_POST["college_type_id"]);
$undertaking_id = sanitize_string($_POST["undertaking_id"]);
$exam_level_id = sanitize_string($_POST["exam_level_id"]);
$exam_type_id = sanitize_string($_POST["exam_type_id"]);
$exam_category_id = sanitize_string($_POST["exam_category_id"]);
...
if ($action == "edit" && !empty($record_id) && isset($record_id)) {
    ...
    if ($existRecord) {
        ...
        $sql = 
            "UPDATE slug_master SET title = :title, response_from = :response_from, course_type_id = :course_type_id, stream_id = :stream_id,college_type_id = :college_type_id,undertaking_id = :undertaking_id,exam_level_id = :exam_level_id,exam_type_id = :exam_type_id,exam_category_id = :exam_category_id,university_id = :university_id, country_id = :country_id, state_id = :state_id, city_id = :city_id,district_id = :district_id,tags = :tags, updated_at = :updated_at,updated_by = :updated_by WHERE id = :id";
        $stmt = $dbConn->prepare($sql);
        ...
        $stmt->bindParam(":exam_level_id", $exam_level_id, PDO::PARAM_INT);
        $stmt->bindParam(":exam_type_id", $exam_type_id, PDO::PARAM_INT);
    ...
    if ($existRecord) {
        ...
        $active_status = 1;
        $sql =
            "INSERT INTO slug_master (title,response_from,course_type_id, stream_id,college_type_id,undertaking_id,exam_level_id,exam_type_id,exam_category_id, university_id,country_id,state_id,city_id,district_id,tags,slug, created_at, created_by) VALUES (:title, :response_from,:course_type_id, :stream_id,:college_type_id,:undertaking_id,:exam_level_id,:exam_type_id,:exam_category_id,:university_id, :country_id, :state_id, :city_id,:district_id,:tags,:slug,:created_at,:created_by)";
        $stmt = $dbConn->prepare($sql);
