...
$exam_category_name = $_POST["exam_category_name"];
...
if ($action == "edit" && !empty($record_id) && isset($record_id)) {
    ...
    if ($Record) {
        ...
        if(isset($_FILES['banner_img']['name']) && !empty($_FILES['banner_img']['name'])){
            ...
            if (empty($errors)) {
                ...
                $slugWithExams = $slug;
                $slugWithoutExams = str_replace("exams/", "", $slugWithExams);
                ...
                $banner_file_name = $slugWithoutExams.'.'.$ext;
        ...
        if(isset($_FILES['logo_img']['name']) && !empty($_FILES['logo_img']['name'])){
            ...
            if (empty($errors)) {
                ...
                $logo_file_name = $slugWithoutExams.'-logo.'.$ext;
               // echo $logo_file_name;
        ...
        if(isset($_FILES['html_page']['name']) && !empty($_FILES['html_page']['name'])){
            ...
            if (empty($errors)) {
                ...
                $page_file_name = $slugWithoutExams.'-page.'.$ext;
               // echo $page_file_name;
        ...
        $sql = 
            "UPDATE exams SET exam_level = :exam_level,exam_type_name = :exam_type_name,exam_category_name = :exam_category_name,country_id = :country_id,state_id = :state_id,banner_img = :banner_img,logo_img = :logo_img,html_page = :html_page, exam_name = :exam_name, about_exam = :about_exam,short_name = :short_name,tags = :tags,updated_at = :updated_at,updated_by = :updated_by WHERE id = :id";
        $stmt = $dbConn->prepare($sql);
    ...
    if ($Record) {
        ...
        $active_status = 1;
        $sql =
            "INSERT INTO exams (exam_name,about_exam,exam_level,exam_type_name,exam_category_name,country_id,state_id,short_name,tags,created_at, created_by, is_active) VALUES (:exam_name,:about_exam,:exam_level,:exam_type_name,:exam_category_name,:country_id,:state_id,:short_name,:tags, :created_at,:created_by, :is_active)";
        $stmt = $dbConn->prepare($sql);
        $stmt->bindParam(":exam_level", $exam_level, PDO::PARAM_STR);
        $stmt->bindParam(":exam_type_name", $exam_type_name, PDO::PARAM_STR);
        $stmt->bindParam(":exam_category_name", $exam_category_name, PDO::PARAM_STR);
