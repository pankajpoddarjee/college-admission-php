...
if ($action == "edit" && !empty($record_id) && isset($record_id)) {
    ...
    if ($Record) {
        ...
        $sql =
            "UPDATE exams SET banner_img = :banner_img,logo_img = :logo_img, exam_name = :exam_name,short_name = :short_name,tags = :tags,updated_at = :updated_at,updated_by = :updated_by WHERE id = :id";
        $stmt = $dbConn->prepare($sql);
    ...
    if ($Record) {
        ...
        $active_status = 1;
        $sql =
            "INSERT INTO exams (exam_name,short_name,tags,created_at, created_by, is_active) VALUES (:exam_name,:short_name,:tags, :created_at,:created_by, :is_active)";
        $stmt = $dbConn->prepare($sql);
        $stmt->bindParam(":exam_name", $exam_name, PDO::PARAM_STR);
        $stmt->bindParam(":short_name", $short_name, PDO::PARAM_STR);
        $stmt->bindParam(":tags", $tags, PDO::PARAM_STR);
        ...
        if ($stmt->execute()) {
            ...
            $sql = "UPDATE exams SET banner_img = :banner_img,logo_img = :logo_img, slug = :slug WHERE id = :id";           
            $stmt = $dbConn->prepare($sql);
            $stmt->bindParam(":banner_img", $banner_img, PDO::PARAM_STR);
            $stmt->bindParam(":logo_img", $logo_img, PDO::PARAM_STR);
